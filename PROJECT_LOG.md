...
### 週結勾選：一鍵複製 LINE 訊息 + 手機 RWD

**情境**：使用者明天（週五）要跟員工收一週餐費，需要在 LINE 群組通知每位同事該付多少。原本要逐個複製訂單金額很慢。

**做法**：
- 沒做 `/admin/reports` 整頁新頁面（規格上是 admin/accountant 看的）
- **直接在現有 `/admin/settlements`（週結勾選）頁加「複製為 LINE 訊息」區塊** — 因為 settlements 頁本來就有「每員工本週累積金額 + 標記已付」資料，差只是格式化 + 複製
- 按鈕在統計卡和員工列表中間，含**預覽框**直接看到要複製的內容
- 複製機制：HTTPS context 用 `navigator.clipboard`，HTTP context (例如 dev server LAN IP) fallback 用 `document.execCommand('copy')`
...
（其他員工...）
──────────
請週五結束前完成付款，完成後請通知訂餐員（付現、Linepay、轉帳皆可）
```
...
**手機 RWD**：
- 表格在手機改成卡片版（hidden table，show ul）
- 卡片內容：姓名 + 金額 + 訂單筆數 + 已付狀態 badge + 標記按鈕
- 已付的卡片邊框變綠色，視覺上一目瞭然
- 週切換條按鈕點擊區域加大（py-2，原本 py-1.5），手機好按

**未做（之後再說）**：
- Excel 匯出（明天結算用不到，先擱置）
- 月區間 / 自訂區間（規格上是 accountant 用，目前還沒這角色）
- `/admin/reports` 獨立頁（功能重疊，先合併在 settlements）
...
**情境**：員工點餐時想附加說明（少糖、不要香菜、餐點變化說明等），希望系統能傳達給訂餐員。

**Schema 變更**（`0008_order_items_note.sql`）：
- `order_items` 加 `note` (text, nullable)

**UX 設計**（兩次迭代）：
- v1（初版）：每個品項各自有備註輸入框 → 使用者反饋「太隱密看不到」
- v2（最終）：每個分類（吃的/喝的）一個共用備註，位置在小計上方、送出按鈕上方 → 同一個 note 套用到該訂單所有 order_items
- 購物車預設展開：cartExpanded 初始值 true，加品項後直接看到備註欄，不用按 🛒

**LIFF**：
- /menu 購物車展開區塊有大的備註輸入框（每分類獨立）
- /orders/[id] 訂單詳細頁顯示備註，編輯 modal 也是訂單共用一個備註
- /today、/history 顯示備註

**後台**：
- /admin/sessions/[id] 叫貨單買家後面顯示備註（同人不同備註會分開列）
- 訂單編輯 modal 每個品項都可改備註（admin 較細）

### 匯入時憶咖哩菜單

- 5 張照片 → AI 辨識 → 產出 INSERT SQL → 37 項全進 DB
- 每個品項加 emoji 區分系列：🥗 小菜 / 🍱 套餐 / 🍛 黑咖哩 / 🍳 溫泉版 / 🧀 起司咖哩
